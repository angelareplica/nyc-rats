...
var version = exports.version = "5.14.1";
...
var svg = d3.select('#grouped-bar-chart').append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');

var colorScale = d3.scaleOrdinal().range(['#657190', '#99A5C6', '#ECD1DF']);

// scale for distance between groups of bars
var xPositionScale0 = d3.scaleBand().rangeRound([0, width]).padding(0.3);

var xPositionScale1 = d3.scaleBand().padding(0.1);

var yPositionScale = d3.scaleLinear().domain([0, 2500]) // bc the max monthly sightings is ~2200, in July 2017
.range([height - margin.top - margin.bottom, 0]);

d3.csv(require('./data/grouped_monthly_count_2017_to_2019.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  xPositionScale1.domain(['2017', '2018', '2019']).range([0, xPositionScale0.bandwidth()]);

  // Grouping bars together + transforming!
  // using translaet to reposition group element w/ margins on left and top
  var monthName = svg.selectAll('.month-name').data(datapoints).enter().append('g').attr('class', 'month-name').attr('transform', function (d) {
    return 'translate(' + xPositionScale0(d.month) + ',0)';
  });

  // add 2017 bars!
  monthName.selectAll('.bars-2017').data(function (d) {
    return [d];
  }).enter().append('rect').attr('class', 'bars-2017').style('fill', '#657190').attr('x', function (d) {
    return xPositionScale1('2017');
  }).attr('y', function (d) {
    return yPositionScale(d['2017']);
  }).attr('width', xPositionScale1.bandwidth()).attr('height', function (d) {
    return height - margin.top - margin.bottom - yPositionScale(d['2017']);
  });

  // add 2018 bars!
  monthName.selectAll('.bars-2018').data(function (d) {
    return [d];
  }).enter().append('rect').attr('class', 'bars-2018').style('fill', '#99A5C6').attr('x', function (d) {
    return xPositionScale1('2018');
  }).attr('y', function (d) {
    return yPositionScale(d['2018']);
  }).attr('width', xPositionScale1.bandwidth()).attr('height', function (d) {
    return height - margin.top - margin.bottom - yPositionScale(d['2018']);
  });

  // add 2019 bars!
  monthName.selectAll('.bars-2019').data(function (d) {
    return [d];
  }).enter().append('rect').attr('class', 'bars-2019').style('fill', '#ECD1DF').attr('x', function (d) {
    return xPositionScale1('2019');
  }).attr('y', function (d) {
    return yPositionScale(d['2019']);
  }).attr('width', xPositionScale1.bandwidth()).attr('height', function (d) {
  ...
  svg.append('g').attr('class', 'axis y-axis').call(yAxis).call(function (g) {
    return g.select(".domain").remove();
  }); // remove y-axis line

  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + (height - margin.top - margin.bottom) + ')') // remove spacing at bottom
  ...
  var legend = svg.selectAll('.legend').data(years).enter().append('g').attr('class', 'legend').attr('transform', function (d, i) {
    return 'translate(0,' + i * 20 + ')';
  });

  legend.append('rect').attr('x', width - 15).attr('width', 12).attr('height', 12).style('fill', function (d) {
    return colorScale(d);
  });

  legend.append('text').attr('x', width - 20).attr('y', 6).attr('dy', '.35em').style('text-anchor', 'end').style('font-size', '10px').style('fill', 'white').text(function (d) {
    return d;
  });
}
},{"d3":"../node_modules/d3/index.js","./data/grouped_monthly_count_2017_to_2019.csv":"data/grouped_monthly_count_2017_to_2019.csv"}],"../../../../.npm-global/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../../../../.npm-global/lib/node_modules/parcel/src/builtins/hmr-runtime.js","grouped-bar-chart.js"], null)
//# sourceMappingURL=rats/grouped-bar-chart.4c835934.map